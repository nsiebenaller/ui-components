...
export default function Autocomplete(props: Props) {
  ...
  return (
    ...
    >
      {
        displayOptions && options.length > 0 && options.map((option, idx) => (
          <Option 
            key={idx} 
            onClick={handleClick(option)}
            centered={props.centered}
          >{valueOf(option)}</Option>
        ))
      }
      {
        displayOptions && options.length === 0 && (
        <Option centered={props.centered} disabled>{props.noMatchText || "none"}</Option>
        )
      }
    </Select>
  );
...
function filterOptions(value: OptionFormat, options: Array<OptionFormat>) : Array<OptionFormat> {
  return options.filter((option) => {
    return valueOf(option).toLowerCase().includes(valueOf(value).toLowerCase())
  })
}
